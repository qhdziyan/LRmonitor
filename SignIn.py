...
class SignInWidget(QWidget):
    ...
    signin_signal = pyqtSignal(str)

    def __init__(self):
        super(SignInWidget, self).__init__()
        self.resize(900, 600)
        self.setWindowTitle("欢迎使用链润科风控管理系统")
        self.setUpUI()
    ...
    def signInCheck(self):
        ...
        query = QSqlQuery()
        sql = "SELECT * FROM riskmornitor.user WHERE username='%s'" % (username)
        query.exec_(sql)
        db.close()
        ...
        if (not query.next()):
            ...
            #if (username == query.value(1) and hl.hexdigest() == query.value(0)):
            if (username == query.value(0) and hl == query.value(1)):
                # 如果是管理员
                if (query.value(3)==1):
                    self.is_admin_signal.emit()
                else:
                    self.signin_signal.emit(username)
            else:
